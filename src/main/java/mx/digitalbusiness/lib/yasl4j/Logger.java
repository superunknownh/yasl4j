...
import static mx.digitalbusiness.lib.yasl4j.utils.Constants.*;
import static mx.digitalbusiness.lib.yasl4j.utils.StringUtils.*;
import static mx.digitalbusiness.lib.yasl4j.utils.SystemUtils.*;

/**
 * This class allow to log messages in different levels for both console and file.
 * <br /><br />
 * In order to work with this logger, please set the next environment variables:
 * <br />
 * <ul>
 *   <li>
 *     {@code YASL4J_LOG_LEVEL} The log level. Options available are:
 *     <ul>
 ...
 *   </li>
 *   <li>
 *     {@code YASL4J_LOG_ROTATE_DAILY} Set to {@code true} if you want to rotate daily in order
 *     to create the file as: {@code $LOG_DIR/$module.yyyy-MM-dd.log} or set to {@code false} for
 *     create a single file as: {@code $LOG_DIR/$module.log}.
 *   </li>
 *   <li>
 *     {@code YASL4J_LOG_DIR} The file system absolute path where the log files will be generated. If not set, the
 *     log to file will not be performed.
 *   </li>
 * </ul>
 *
 * @author Hugo
...
public class Logger<T> {
	 ...
	 * 
	 * @param rotateDaily {@code true} for rotate daily, {@code false} for create a single file.
	 */
	...
	private void log(LogLevel level, String module, String submodule, Object message) {
		...
		if (level.getLevel() >= this.logLevel.getLevel()) {
			String logMessage = String.format("%s [%s] [%s] %s - %s", getCurrentDateTime(), level.getDescription(), module, submodule, message.toString());
